...
void APIENTRY_GL4ES gl4es_glBindBuffer(GLenum target, GLuint buffer) {
    ...
    if (buffer == 0) {
        ...
        DBG(printf("glBindBuffer %d => %d\n", buffer, buff->real_buffer);)
    }
    noerrorShim();
}

void APIENTRY_GL4ES gl4es_glBindBufferBase(GLenum target, GLuint index, GLuint buffer) {
    DBG(printf("glBindBufferBase(%s, %i, %u)\n", PrintEnum(target), index, buffer);)
    LOAD_GLES2(glBindBufferBase)
    //gles_glBindBufferBase(target, index, buffer);
}

void APIENTRY_GL4ES gl4es_glBufferStorage(GLenum target, GLsizeiptr size, const GLvoid * data, GLbitfield flags) {
    DBG(printf("glBufferStorage(%s, %zi, %p, 0x%04X)\n", PrintEnum(target), size, data, flags);)
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (t) {    // don't allow to remove default one
                ...
                if (k != kh_end(list)) {
                    ...
                    if (glstate->vao->read == buff)
                        glstate->vao->read = NULL;
                    if (glstate->vao->write == buff)
                        glstate->vao->write = NULL;
                    for (int j = 0; j < hardext.maxvattrib; j++)
...
void APIENTRY_GL4ES gl4es_glReadBuffer(GLenum mode) {
    DBG(printf("glReadBuffer(%s)\n", PrintEnum(mode));)
    LOAD_GLES2(glReadBuffer);
...
void deleteSingleBuffer(GLuint buffer) {
   ...
   else if(glstate->bind_buffer.copy_read == buffer) glstate->bind_buffer.copy_read = 0;
   else if(glstate->bind_buffer.copy_write == buffer) glstate->bind_buffer.copy_write = 0;
...
AliasExport(void,glBufferStorage,,(GLenum target, GLsizeiptr size, const GLvoid * data, GLbitfield flags));
...
AliasExport(void,glReadBuffer,,(GLenum mode));
