...
static const char *gles2_angle_lib[] = {
    "libGLESv2_angle",
    NULL
};
...
static const char *gles_angle_lib[] = {
    "libGLESv1_CM_angle",
    NULL
};
...
static const char *egl_angle_lib[] = {
    "libEGL_angle",
    NULL
};
...
void load_libs() {
...
#endif
    bool useAngle = strcmp(getenv("LIBGL_BACKEND_ANGLE"), "1") == 0;
    gles = open_lib((globals4es.es==1)?(useAngle?gles_angle_lib:gles_lib):(useAngle?gles2_angle_lib:gles2_lib), gles_override);
#else
    ...
    egl = open_lib(useAngle?egl_angle_lib:egl_lib, egl_override);
#else
