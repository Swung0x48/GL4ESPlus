        ...
        if (intercept) {
            ...
            if (mode==GL_QUADS) {
                ...
                if((indcnt < realcount) || (indfirst!=realfirst)) {
                    ...
                    indfirst = realfirst;
                    GLushort *p = indices;
                    for (int h=0, j=indfirst; h+3<indcnt; h+=4, j+=4) {
                            *(p++) = j + 0;
                            *(p++) = j + 1;
                            *(p++) = j + 2;
...
void APIENTRY_GL4ES gl4es_glMultiDrawElements( GLenum mode, GLsizei *counts, GLenum type, const void * const *indices, GLsizei primcount)
{
    ...
    for (int i=0; i<primcount; i++) {
        ...
        if(need_free) {
            ...
            if(type==GL_UNSIGNED_INT)
                iindices = (glstate->vao->elements)?((void*)((char*)glstate->vao->elements->data + (uintptr_t)indices[i])):(GLvoid*)indices[i];
            else
                sindices = (glstate->vao->elements)?((void*)((char*)glstate->vao->elements->data + (uintptr_t)indices[i])):(GLvoid*)indices[i];
        }
